...
// FORK (tengrids): story helper modules (data-editor/stories/utils.tsx,
// stories/story-utils.tsx, docs/doc-wrapper.tsx) use @linaria/react's
// `styled` and @linaria/core's `css`, which require a build-time transform
// esbuild doesn't run - linaria's real dist throws "Using the styled tag in
// runtime is not supported" the moment a helper module evaluates. This stub
// reimplements the runtime semantics linaria compiles to: one generated
// class per template (raw template injected as native nested CSS - the
// capture browser and claude.ai/design targets are modern Chromium), and
// function interpolations become CSS custom properties set from props via
// inline style, exactly linaria's own runtime strategy.
const LINARIA_STUB =
  'var seq=0;' +
  'function inject(t){if(typeof document!=="undefined"){var s=document.createElement("style");s.textContent=t;document.head.appendChild(s);}}' +
  'function build(tag){return function(strings){' +
  'var interps=Array.prototype.slice.call(arguments,1);' +
  'var id=++seq,cls="ds-lin-"+id,cssText="",fns=[];' +
  'for(var i=0;i<strings.length;i++){cssText+=strings[i];' +
  'if(i<interps.length){var v=interps[i];' +
  'if(typeof v==="function"&&!v.__linCls){var vn="--ds-lin-"+id+"-"+i;fns.push([vn,v]);cssText+="var("+vn+")";}' +
  'else if(v&&v.__linCls){cssText+="."+v.__linCls;}' +
  'else{cssText+=(v==null?"":String(v));}}}' +
  'inject("."+cls+"{"+cssText+"}");' +
  'var C=function(props){var react=window.React;' +
  'var style=props.style;' +
  'if(fns.length){style=Object.assign({},style);for(var j=0;j<fns.length;j++){try{var val=fns[j][1](props);if(val!=null)style[fns[j][0]]=String(val);}catch(e){}}}' +
  'var p=Object.assign({},props,{className:(props.className?props.className+" ":"")+cls});' +
  'if(style)p.style=style;' +
  'var as=p.as;delete p.as;' +
  'return react.createElement(as||tag,p);};' +
  'C.__linCls=cls;C.displayName="Styled("+(typeof tag==="string"?tag:(tag&&(tag.displayName||tag.name))||"c")+")";' +
  'return C;};}' +
  'var styled=new Proxy(function(tag){return build(tag)},{get:function(t,k){if(typeof k==="string"&&k!=="prototype"&&k!=="call"&&k!=="apply")return build(k);return t[k];}});' +
  'function css(strings){var interps=Array.prototype.slice.call(arguments,1);' +
  'var cls="ds-lin-"+(++seq),t="";' +
  'for(var i=0;i<strings.length;i++){t+=strings[i];if(i<interps.length){var v=interps[i];t+=(v&&v.__linCls)?"."+v.__linCls:(v==null?"":String(v));}}' +
  'inject("."+cls+"{"+t+"}");return cls;}' +
  'function cx(){return Array.prototype.slice.call(arguments).filter(Boolean).join(" ");}' +
  'module.exports={styled:styled,css:css,cx:cx};';

export function linariaStubPlugin() {
  return {
    name: 'linaria-stub',
    setup(b) {
      b.onResolve({ filter: /^@linaria\/(react|core)$/ }, (a) => ({ path: a.path, namespace: 'linaria-stub' }));
      b.onLoad({ filter: /.*/, namespace: 'linaria-stub' }, () => ({ contents: LINARIA_STUB, loader: 'js' }));
    },
  };
}
...
export function storyImportPlugins({ PKG, GLOBAL, extraEntries = [], exported, cfg, pkgDir }) {
  ...
  return {
    plugins: [dsShim, storybookStubPlugin(), linariaStubPlugin(), consoleStub, policyRedirect],
