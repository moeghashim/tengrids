...
- `npm audit --omit=dev` is kept clean, with one caveat: the `dompurify` override in the root `package.json` exists because `@toast-ui/editor` (the ArticleCell in `packages/cells`) inlines DOMPurify 2.3.3 into its dist, so the override only fixes the unused on-disk copy. Grid content is trusted by design (core markdown cells also render `marked` output unsanitized, as upstream does); the real fix for the article cell is replacing toast-ui. Also note that npm 11.12 does not apply an override to a package that is already in the lockfile; if `dompurify` 2.x ever reappears, remove its lockfile entries and reinstall with npm 10 (`npx npm@10 install`), which honors the override.
