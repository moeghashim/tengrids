...
- `packages/cells`, `packages/source`, `packages/schema`, and `packages/ai` tests import core's **built** `dist/` — build core first (`npm run build -w packages/core`) or their suites fail to resolve `tengrids`. `tengrids-ai` also needs `packages/schema/dist`. `packages/mcp` is a Node CLI (ESM-only `tsc` to `dist/`); its tests load the docs bundle from `packages/mcp/docs/` (generated by `docs-bundle`) and the C1 stdio test spawns `dist/cli.js`.
...
| Cells / source / schema / ai / mcp tests | `npm run test-cells` / `npm run test-source` / `npm run test-schema` / `npm run test-ai` / `npm run test-mcp` | ai: 126 tests, ~1 s; schema: 48; mcp: ≥ 30                                                                                             |
| Docs bundle                              | `npm run docs-bundle`                                                                                         | Writes `packages/mcp/docs/` (gitignored JSON + `llms-full.txt`) from API.md, package READMEs, AGENTS.md, llms.txt, and `*.stories.tsx` |
...
CI gates on (`node.js.yml`): build, core tests with coverage, source tests, cells tests, ai tests, schema tests, mcp tests, downstream consumer projects (`test-projects/` — CRA5 and Next, which depend on core via `file:` links), a C2 step that typechecks `scaffold` output in `test-projects/next-gdg`, and the full suite re-run against React 18, 19, and latest. `visual.yml` runs the screenshot job; `storybook.js.yml` runs `docs-bundle` then deploys Storybook plus `API.md`, `llms.txt`, and `llms-full.txt` to GitHub Pages (https://moeghashim.github.io/tengrids/). Keep all of these green.
...
- `packages/mcp` — `tengrids-mcp`: stdio MCP server (`npx -y tengrids-mcp`) over bundled docs. Tools: `list_docs`, `get_doc`, `search_docs`, `get_example`, `scaffold`, `check_setup`. ESM-only, Node 20.10+, no React in this package.
