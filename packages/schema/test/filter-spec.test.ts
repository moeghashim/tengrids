...
import { evaluateFilter, findColumnIndex, matchesClause, specColumns } from "../src/index.js";

const t = (s: string): GridCell => ({ kind: GridCellKind.Text, data: s, displayData: s, allowOverlay: false });
const n = (v: number): GridCell => ({
...
describe("evaluateFilter / specColumns / findColumnIndex", () => {
    it("and/or semantics and unknown columns", () => {
        const and = {
            ...
            clauses: [
                { column: "dept", op: "eq" as const, value: "engineering" },
                { column: "Age", op: "gte" as const, value: 30 },
            ],
        };
        const or = {
            ...
            clauses: [
                ...
                { column: "Active", op: "eq" as const, value: true },
            ],
        };
    ...
    it("findColumnIndex matches title or id case-insensitively", () => {
        expect(findColumnIndex(columns, "DEPT")).toBe(1);
        expect(findColumnIndex(columns, "Name")).toBe(0);
        expect(findColumnIndex(columns, "missing")).toBe(-1);
    });
});
